...
## Nonce Disambiguation {#nonce-disambiguation}
...
In this interaction model, that nonce is an attestation nonce (freshness handle) as described in {{nonce-disambiguation}}.
...
In the Passport model, the Handle MAY be generated by the Relying Party and conveyed as part of the `requestEvidence(...)` call; alternatively, it MAY be generated by the Verifier and conveyed to the Attester (e.g., via the Relying Party or other protocol mechanisms).
...
In the Uni-Directional model, Evidence may be conveyed as full Evidence or as delta Evidence. Regardless of whether delta Evidence is used, each Evidence conveyance is bound to a current Handle.
In the diagrams, `handleDelta` denotes the Handle associated with the delta Evidence conveyance, which may be the same as the previous Handle or a newly issued Handle (e.g., for a new Epoch or freshness window).
The concrete rules for when a Handle is rotated (resulting in a new `handleDelta`) are deployment- and protocol-specific and are out of scope of this document.

Note: If a Verifier does not receive the current Handle (e.g., due to propagation loss or delay), it may be unable to determine Evidence freshness for the corresponding Epoch.
In such cases, Evidence appraisal cannot be completed until re-synchronization occurs.
Error signaling and re-synchronization mechanisms are protocol-specific and out of scope of this document.
...
Delta Evidence is produced with respect to the previous conveyance, but it is still bound to a (potentially updated) current Handle (e.g., `handleDelta`).
If a new Handle is required, it is conveyed by a subsequent subscribe operation.
...
Attestation Results conveyed to Relying Parties MUST be bound to the Handle used for the corresponding Evidence appraisal (to prevent mix-up and replay across Handles/Epochs, and to enable correlation).
This binding can be achieved by including the Handle in the Attestation Result itself, or by cryptographically binding the Handle to the published message (e.g., signing a structure that includes both Handle and Attestation Result).
...
In {{fig-coserv-transaction}}, the RATS roles of Verifier, Endorser and Reference Value Provider can take on an additional *logistic* role of CoSERV producer and consumer.
